          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-8 relative z-10">
            {steps.map((step, index) => (
              <div key={index} className="group flex flex-col items-center">
                {/* Step Circle */}
                <div className="w-20 h-20 bg-white border-4 border-blue-50 rounded-full flex items-center justify-center text-blue-600 shadow-sm group-hover:shadow-md group-hover:border-blue-500 transition-all duration-300 mb-6 relative">
                  {step.icon}
                  {/* Step Number Badge */}
                  <span className="absolute -top-2 -right-2 bg-blue-600 text-white text-xs font-bold w-6 h-6 rounded-full flex items-center justify-center shadow-lg">
                    {index + 1}
                  </span>
                </div>
